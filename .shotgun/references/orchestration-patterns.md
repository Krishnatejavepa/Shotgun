...
Plugin subagents go in `.shotgun/agents/` at the plugin root. This repo is a plugin (`.claude-plugin/plugin.json`), so `.shotgun/agents/code-reviewer.md`, `.shotgun/agents/security-auditor.md`, and `.shotgun/agents/test-engineer.md` are auto-discovered when the plugin is enabled. No path configuration needed.
...
Plugin subagents do **not** support the `hooks`, `mcpServers`, or `permissionMode` frontmatter fields — these are silently ignored. If a future persona needs any of those, the user must copy the file into `.shotgun/agents/` or a user-level agents directory instead.
...
Agent Teams is experimental. In `~/.shotgun/settings.json`:
