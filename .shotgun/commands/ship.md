...
Invoke the shipping-and-launch skill (`.shotgun/skills/shipping-and-launch/SKILL.md`).
...
In agents that support subagent spawning (e.g., Claude Code, Antigravity), each call passes `subagent_type` matching the persona's `name` field:
...
- If you need teammates that talk to each other instead of just reporting back, use your agent's team features and reference these personas as teammate types (see `.shotgun/references/orchestration-patterns.md`).

**Persona resolution.** If you've defined your own `code-reviewer`, `security-auditor`, or `test-engineer` in `.shotgun/agents/` or a user-level agents directory, those take precedence over this plugin's versions — `/ship` picks up your customizations automatically.
