...
Invoke the incremental-implementation skill (`.shotgun/skills/incremental-implementation/SKILL.md`) alongside the test-driven-development skill (`.shotgun/skills/test-driven-development/SKILL.md`).
...
3. **Plan if needed.** If there is no `tasks/plan.md`, invoke the planning-and-task-breakdown skill (`.shotgun/skills/planning-and-task-breakdown/SKILL.md`) to generate one.
...
6. **Stop and ask the user** (do not push through) when:
   - a test can't be made to pass or the build breaks without an obvious fix → follow the debugging-and-error-recovery skill (`.shotgun/skills/debugging-and-error-recovery/SKILL.md`)
   - the spec is ambiguous, or a task needs a decision the spec doesn't cover
   - a task is high-risk or irreversible — auth/permission changes, destructive data migrations, payments, deletions, deploys, anything touching secrets, **or anything you can't undo with `git revert`** → follow the doubt-driven-development skill (`.shotgun/skills/doubt-driven-development/SKILL.md`) and get explicit sign-off before continuing
...
If any step fails, follow the debugging-and-error-recovery skill (`.shotgun/skills/debugging-and-error-recovery/SKILL.md`).
