...
If the venture clearly matches a starter in `templates/` (SaaS, creator, agency, e-commerce), use that template's skeleton and vault taxonomy as the starting shape, it front-loads the questions that matter for that business type. No match = plain template, never force a fit.
...
If the founder mentions an existing Obsidian vault, Notion workspace, ChatGPT/Claude export, or a previous Shotgun copy, run the import flow from `.claude/skills/port/SKILL.md` as part of Phase 3.
