...
1. Every `.claude/skills/*/SKILL.md` has frontmatter with `name` and `description`. Broken frontmatter = a skill Claude Code can't discover → fix it.
...
`git log --oneline -5 -- CLAUDE.md`: recent changes to CLAUDE.md get shown to the founder with one question: "You approved these, right?" (§9.5: the loop is never edited silently.)
